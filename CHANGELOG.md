...
+ [修复] 文本模型的视觉输入在请求前按总预算压缩；图片反推改为八维拆解并输出结构化中文生图提示词，后续生图强制继承原图且禁止无 image-ref 时退化为纯文字生成；AI 代理允许较大请求，并将 Nginx 413 HTML 错误页转换为明确的中文提示。
+ [修复] 生成 3 个剧情钩子前校验产品参考图和文本模型的 image-ref 能力，避免无参考图或错误模型配置时发起无效多模态请求。
+ [新增] 增加登录、管理员创建用户、服务器加密保存模型渠道、普通用户受控调用和每日额度限制，并将产品显示名称调整为“一格画布”。
+ [调整] 生产镜像改由 Bun 同时托管前端、SQLite 与 AI 代理，继续沿用单容器健康检查、双端口切换和失败回滚。
+ [新增] 增加用户自有 GHCR 开发镜像、`develop` 自动部署、双端口健康检查、Nginx 切换与失败回滚草案，并显示构建 Git SHA。
+ [新增] 视频节点支持抽取本地关键帧并使用文本模型生成「对标视频拆解」文本节点，结果自动连回原视频节点。
+ [恢复] 对标拆解结果节点恢复产品资料、3 个剧情钩子、完整销售剧本、结构化分镜、角色基准图和完整故事板工作流。
+ [修复] 恢复 YigeAI 本地代理，并让 OpenAI 兼容渠道中的 Gemini 文本模型改用 Chat Completions 接口。
